...
% transformar a datos por cinco años promedio
% Transformar cada tabla a datos quinquenales
Pre_Arahuay = transform_table_quinquennial(Pre_Arahuay);
Pre_Authisha = transform_table_quinquennial(Pre_Authisha);
Pre_Canchacalla = transform_table_quinquennial(Pre_Canchacalla);
Pre_Carampoma = transform_table_quinquennial(Pre_Carampoma);
Pre_Matucana = transform_table_quinquennial(Pre_Matucana);
Pre_RioBlanco = transform_table_quinquennial(Pre_RioBlanco);
Pre_SantaEula = transform_table_quinquennial(Pre_SantaEula);
Pre_SantTuna = transform_table_quinquennial(Pre_SantTuna);
Pre_Nana = transform_table_quinquennial(Pre_Nana);
...
Pre_Chosica = transform_table_quinquennial(Pre_Chosica);
...
% Función para transformar la tabla a datos anuales promedio
function data = transform_table_annual(T)
    ...
    annual_precipitation_mean = sum(data_array, 2)/12;
    
    % Crear una nueva tabla con datos anuales
    data = table(years, annual_precipitation_mean, 'VariableNames', {'Year', T.Properties.VariableNames{2}});
end
%función para trasformar la tabla a datos por cinco años promedio

% Función para transformar la tabla a datos quinquenales
function data = transform_table_quinquennial(T)
    % Convertir la tabla en un array para facilitar la manipulación
    data_array = table2array(T(:, 2:end-1));  % Ignorar la columna de Año y Total Anual
    years = T{:, 1};  % Obtener los años
    ...
    quinquennial_precipitation = [];
    quinquennial_years = [];
    
    % Calcular el promedio quinquenal
    for i = 1:5:num_years
        if i + 4 <= num_years
            quinquenio = data_array(i:i+4, :);
            quinquenio_mean = mean(sum(quinquenio, 2));  % Promedio de la suma anual
            quinquennial_precipitation = [quinquennial_precipitation; quinquenio_mean];
            quinquennial_years = [quinquennial_years; years(i)];
        end
    end
    
    % Crear una nueva tabla con datos quinquenales
    data = table(quinquennial_years, quinquennial_precipitation, 'VariableNames', {'Year', T.Properties.VariableNames{2}});
end